...
const char *fragment_frag =
 ...
 "{"
   ...
   "float s=y<32?"
     "4:"
     "1,r=circle(x*2.45,1.,s)*1.12-circle(x*2.85,.7,s)*.21;"
   "r-=circle(vec2(x.x-sin(3)*.85,1.8*x.y-cos(3)*.65)*.8,1.,s)*.1;"
   "vec3 z=vec3(r)*vec3(1,0,.5)+vec3(smoothstep(.2,.7,r))*vec3(1,1,0)+vec3(smoothstep(.4,.55,r));"
   "f.xyz=z+i.xyz*.2+i.xyz*vec3(.7*h(x*1.1),0.*h(x*1.7),.9*h(x*2.2));"
 "}"
 "float r(vec2 v)"
 "{"
